...
class Basket {
    ...
    showBasket(){
        ...
        this.good.forEach(item => { document.getElementById('listOfCart').innerHTML += `<div class="basketCart"><p>${item.product_name}</p><br><button >&#215;</button></div> `}) //onclick='bask.removeBasketCart(${this.parentNode})'
        document.querySelectorAll('#head .cart-button button').addEventListener('click', this.removeBasketCart(e)) // ВОТ тут не могу никак событие словить. Как обратиться правильно к кнопке?
    }
