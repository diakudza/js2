...
class Basket {
    ...
    showBasket() {
        ...
        }, true)) // Как обратиться правильно к кнопке? Event вешается не только на кнопки. что я в селекторе ищу, но и на карточки с товаром в блок .good-list . Так же не понял, как остановить ё
        this.totalSum()
        this.goodsCount()
        document.querySelectorAll('.goods-list div').forEach(div => {removeEventListener('click', (event) => {
            this.removeBasketCart(event)
        }, true)}) //вот этим пытаюсь победить снятие евентлистенера с товаров карточек, но что-то через раз работает.
    }
